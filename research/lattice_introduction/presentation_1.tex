...
\begin{frame}
    \frametitle{Gram-Schmidt for Lattices - LLL Reduction}
    \begin{block}<1->{$\delta-$LLL Reduced basis}
        A basis $\{ \mathbf{b}_1, \mathbf{b}_2, \dots , \mathbf{b}_n \} $ is called LLL-reduced if and only if:
        \begin{enumerate}
            \pause \item $\forall i \neq j$, $|\mu_{i,j}| \leq \frac{1}{2}$, where $\mu_{i,j} = \frac{\langle \tilde{\mathbf{b}}_i, \mathbf{b}_j \rangle}{\langle \tilde{\mathbf{b}}_j, \tilde{\mathbf{b}}_j \rangle}$.
            \pause \item $\forall 1 \leq i \leq n$, $\delta \norm{\tilde{b}_i}^2 \leq \norm{\mu_{i+1,i}\tilde{b}_i + \tilde{b}_{i+1} }^2 $
        \end{enumerate}
    \end{block}
...
\begin{frame}
    ...
    \FOR{$i=1$ to $N$}
    \FOR{$j=i-1$ to $1$}
    \STATE $b_i =$ $b_i - \lfloor c_{i, j} \rceil b_j$, $c_{i, j} = \frac{\langle \tilde{\mathbf{b}}_i, \mathbf{b}_j \rangle}{\langle \tilde{\mathbf{b}}_j, \tilde{\mathbf{b}}_j \rangle}$
    \ENDFOR

    \STATE \emph{Swap Step}:
    \IF {$\exists i$ s.t. $\delta \norm{\tilde{b}_i}^2 > 
        \norm{\mu_{i+1,i}\tilde{b}_i + \tilde{b}_{i+1} }^2 $} 
