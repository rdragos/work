...
\author[R.F.Olimid, D.A.Rotaru]{Ruxandra F. Olimid and \textbf{Dragos Alin Rotaru}} % Your name
...
\begin{frame}
    \frametitle{ID-Based Crypto}
    \begin{figure}
        \includegraphics[width=5cm,height=3cm,keepaspectratio]{wiki.png}
        \source{http://en.wikipedia.org/wiki/ID-based_encryption}
        \caption{ID-Based Crypto}
    \end{figure}
...
\begin{frame}
    ...
    \only<4->{
        \begin{textblock*}{6cm}(4cm, 2cm)
            We have a common secret key: $F(k_{Alice}, (Alice, Bob))$
        \end{textblock*}
    }
...
\begin{frame}
    \begin{figure}
        \includegraphics[width=8cm,height=7.5cm,keepaspectratio]{protocol.png}
        \source{xkcd.com}
        \caption{Protocol}
    \end{figure}
\end{frame}
