...
(* The reduce function defines the one step relation.
...
let rec reduce = function
  ...
  | (Inc l, s) ->
      Some (Secv(Atrib(l, Op(Loc l, Plus, Int 1)), Op(Loc l, Plus, Int(-1))), s) (*x++*)
  | _ -> None                                                      (*default*)
...
(*
 * 1. b)
 * To see the results run: "evaluate (config_inm 2 3);;"
*)
let config_inm x y =
...
let factorial =
  Secv(Atrib("f", Int 1),
    While(Op(Int 1, Mic, Loc "n"),
      Secv(Atrib("x", Loc "f"),
      Secv(Atrib("y", Loc "n"),
      Secv(inmulteste,
      Secv(Atrib("f", Loc "z"),
      Atrib("n", Op(Loc "n", Plus, Int (-1))
      ))))))
  );;

let fact_pgm n =
  evaluate (factorial, [("f", 0); ("n", n); ("x", 0); ("y", 0); ("z", 0)]);;
