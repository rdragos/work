...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> area;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << area << std::endl;
		std::cout.flush();

		__timer_start();

		HouseBuilding __object;
		int __actual = __object.getMinimum(area);
