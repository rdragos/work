...
string	target;
		...
		__in >> __expected >> begin >> target;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << begin << ", " << target << std::endl;
		std::cout.flush();

		__timer_start();

		FoxAndChess __object;
		string __actual = __object.ableToMove(begin, target);
