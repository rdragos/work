...
int	badStep;
		...
		__in >> __expected >> N >> badStep;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << badStep << std::endl;
		std::cout.flush();

		__timer_start();

		JumpFurther __object;
		int __actual = __object.furthest(N, badStep);
