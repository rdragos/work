...
vector <int>	keys;
		...
		__in >> __expected >> doorR >> doorG >> roomR >> roomG >> roomW >> keys;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << doorR << ", " << doorG << ", " << roomR << ", " << roomG << ", " << roomW << ", " << keys << std::endl;
		std::cout.flush();

		__timer_start();

		KeyDungeonDiv1 __object;
		int __actual = __object.maxKeys(doorR, doorG, roomR, roomG, roomW, keys);
