...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> field;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << field << std::endl;
		std::cout.flush();

		__timer_start();

		GameInDarknessDiv1 __object;
		string __actual = __object.check(field);
